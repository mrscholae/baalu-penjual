    ...
    $(document).on("click", ".prioritasToko", function(){
        let form = "#prioritasToko";
        let id_toko = $(this).data("id");
        ...
        $(form+" input[name='prioritas']:checkbox").prop('checked',false);
        $(form+" input[name='id_toko']").val(result.id_toko);
        $(form+" input[name='nama_toko']").val(result.nama_toko);

        result = ajax(url_base+"toko/get_prioritas", "POST", data)
        
        if(result){
            if(result.jarak == 1) $(form+" #jarak").prop("checked", true)
            if(result.pelayanan == 1) $(form+" #pelayanan").prop("checked", true)
            if(result.repeat_order == 1) $(form+" #repeat_order").prop("checked", true)
            if(result.retur == 1) $(form+" #retur").prop("checked", true)
            if(result.pengunjung == 1) $(form+" #pengunjung").prop("checked", true)
            if(result.min_order == 1) $(form+" #min_order").prop("checked", true)
        }
    })

    $(document).on("click", "#prioritasToko .btnSimpan", function(){
        let form = "#prioritasToko";

        let id_toko = $(form+" input[name='id_toko']").val();
        
        Swal.fire({
            icon: 'question',
            text: 'Yakin akan menginputkan prioritas toko?',
            showCloseButton: true,
            showCancelButton: true,
            confirmButtonText: 'Ya',
            cancelButtonText: 'Tidak'
            ...
            if (result.value) {
                ...
                prioritas = new Array();
                $.each($("input[name='prioritas']:checked"), function(){
                ...
                // if(prioritas.length != 0){
                    ...
                    if(result == 1){
                        ...
                        Swal.fire({
                            position: 'center',
                            icon: 'success',
                            text: 'Berhasil menginputkan prioritas toko',
                            showConfirmButton: false,
                            timer: 1500
                        })
                    } else {
